...
class result_Class:
    def __init__(self, root):
        ...
        Label(self.root, text="Add Student Result ", font=("goudy old style", 20, "bold"), bg='orange', fg="#262626",
        ...
        lbl_full_marks = Label(self.root, text="Full Mark", font=("goudy old style", 15, 'bold'), bg='white').place(x=50, y=340)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get() == "":
            ...
            elif self.var_name.get() == "Select" or self.var_marks.get() == "" or self.var_full_marks.get() == "":
                messagebox.showerror("Error", "Please fill the record first", parent=self.root)
            elif  int(self.var_full_marks.get()) != 100:
                messagebox.showerror("Error", "Maximum value exceeded", parent=self.root)
                ...
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "Result name already present", parent=self.root)
                else:
                    per = round(int(self.var_marks.get()) * 100 / int(self.var_full_marks.get()))
