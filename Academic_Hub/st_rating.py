...
class Course_Class:
    def __init__(self, root):
        self.root = root
        self.root.title("Give your feed_back")
        self.root.geometry("1200x480+80+170")
        self.root.config(bg="white")
        self.root.focus_force()

        # Create the title label================
        Label(self.root, text="Your feedback matters a lot", font=("goudy old style", 20, "bold"), bg='orange',
              fg="#262626",
              cursor="hand2").place(x=10, y=15, width=1180, height=35)

        #=======varables=======
        self.var_roll=StringVar()#roll<-----course
        self.var_duration = StringVar()
        self.var_charges = StringVar()

        #====Widgets======
        lbl_Email=Label(self.root,text="Course Name",font=("goudy old style",15,'bold'),bg='white').place(x=10,y=60)
        lbl_courseName = Label(self.root, text="Duration", font=("goudy old style", 15, 'bold'), bg='white').place(x=10, y=100)
        lbl_study = Label(self.root, text="Charges", font=("goudy old style", 15, 'bold'), bg='white').place(x=10, y=140)
    ...
    def delete(self):
        ...
        try:
            if self.var_roll.get()=="":
                messagebox.showerror("Error","Course name shold be required ",parent=self.root)
            else:
    ...
    def get_data(self,ev):
        ...
        #row=content["values"]
        try:
            row = self.CourseTable.item(self.CourseTable.selection())['values']
            if row:
